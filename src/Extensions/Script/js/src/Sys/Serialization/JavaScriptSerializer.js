...
Sys.Serialization.JavaScriptSerializer._init = function () {
  ...
  for (var i = 0; i < 32; i++) {
    ...
    Sys.Serialization.JavaScriptSerializer._charsToEscapeRegExs[c] = new RegExp(
      c,
      "g",
    );
      ...
      for (var i = 0; i < 34; i++) {
        ...
        if (string.indexOf(c) !== -1) {
          if (
            ...
            string = string
              .split(c)
              .join(Sys.Serialization.JavaScriptSerializer._escapeChars[c]);
          } else {
            string = string.replace(
          ...
          for (var j = 0; j < prevObjects.length; j++) {
            if (prevObjects[j] === object) {
              throw Error.invalidOperation(
                Sys.Res.cannotSerializeObjectWithCycle,
              );
            }
          }
        } else {
