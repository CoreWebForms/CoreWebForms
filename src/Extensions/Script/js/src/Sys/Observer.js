...
Sys.Observer._removeEventHandler = function (target, eventName, handler) {
  Sys.Observer._getContext(target, true).events._removeHandler(
    eventName,
    handler,
  );
};
...
Sys.Observer._setValue = function (target, propertyName, value) {
  ...
  for (var i = 0, l = path.length - 1; i < l; i++) {
    ...
    if (target === null || type === "undefined") {
      throw Error.invalidOperation(
        String.format(Sys.Res.nullReferenceInPath, propertyName),
      );
    }
  }
  var currentValue,
...
Sys.Observer.raisePropertyChanged = function (target, propertyName) {
  ...
  Sys.Observer.raiseEvent(
    target,
    "propertyChanged",
    new Sys.PropertyChangedEventArgs(propertyName),
  );
};
...
Sys.Observer.add = function (target, item) {
  ...
  var change = new Sys.CollectionChange(
...
Sys.Observer.raiseCollectionChanged = function (target, changes) {
  ...
  Sys.Observer.raiseEvent(
    target,
    "collectionChanged",
    new Sys.NotifyCollectionChangedEventArgs(changes),
  );
};
...
Sys.Observer._observeMethods = {
  ...
  raisePropertyChanged: function (name) {
    ...
    Sys.Observer.raiseEvent(
      this,
      "propertyChanged",
      new Sys.PropertyChangedEventArgs(name),
    );
  },
};
