...
Sys._Application.prototype.addHistoryPoint = function (state, title) {
  ...
  for (var n in state) {
    ...
    if (
      v !== null &&
      (t === "object" || t === "function" || t === "undefined")
    ) {
...
Sys._Application.prototype._onPageRequestManagerEndRequest = function (
  sender,
  args,
) {
  ...
  if (this._enableHistory) {
    ...
    if (entry !== this._currentEntry) {
      ...
      if (window.theForm) {
        ...
        window.theForm.action =
          (hashIndex !== -1 ? action.substring(0, hashIndex) : action) +
          "#" +
          entry;
      }
