...
Sys.WebForms.PageRequestManager.prototype = {
  ...
  _commitControls: function (updatePanelData, asyncPostBackTimeout) {
    ...
    if (
      typeof asyncPostBackTimeout !== "undefined" &&
  ...
  _createPanelID: function (panelsToUpdate, postBackSettings) {
    var asyncTarget = postBackSettings.asyncTarget,
      toUpdate = this._ensureUniqueIds(
        panelsToUpdate || postBackSettings.panelsToUpdate,
      ),
      panelArg =
        toUpdate instanceof Array
          ? toUpdate.join(",")
  ...
  _doPostBack: function (eventTarget, eventArgument) {
    ...
    if (!event) {
      ...
      if (caller) {
        ...
        event =
          recursionLimit && caller.arguments.length
            ? caller.arguments[0]
            : null;
      }
    }
    ...
    if (
      ...
      if (!postBackElement && mpUniqueID) {
        if (eventTarget.indexOf(mpUniqueID + "$") === 0) {
          ...
          // of the client ID.
          postBackElement = document.getElementById(
            clientID.substr(mpUniqueID.length + 1),
          );
        }
      }
      if (!postBackElement) {
          ...
          if (Array.contains(this._postBackControlIDs, eventTarget)) {
            ...
            if (nearestUniqueIDMatch) {
              ...
              if (mpUniqueID) {
                mpUniqueID += "$";
                if (eventTarget.indexOf(mpUniqueID) === 0) {
                  nearestUniqueIDMatch = this._findNearestElement(
                    eventTarget.substr(mpUniqueID.length),
                  );
                }
              }
              if (nearestUniqueIDMatch) {
                ...
                function testCausesPostBack(attr) {
                  ...
                  return (
                    (causesPostback.test(attr) &&
                      attr.indexOf("'" + eventTarget + "'") !== -1) ||
                    attr.indexOf('"' + eventTarget + '"') !== -1
                  );
                }
                if (
  ...
  _endPostBack: function (error, executor, data) {
    ...
    if (handler) {
      var eventArgs = new Sys.WebForms.EndRequestEventArgs(
        error,
        data ? data.dataItems : {},
        executor,
      );
  ...
  _ensureUniqueIds: function (ids) {
    ...
    for (var i = 0, l = ids.length; i < l; i++) {
      var id = ids[i],
        index = Array.indexOf(this._updatePanelClientIDs, id);
  ...
  _getPageLoadedEventArgs: function (initialLoad, data) {
    ...
    return new Sys.WebForms.PageLoadedEventArgs(
      updated,
      created,
      data ? data.dataItems : {},
    );
  ...
  _getPageLoadingEventArgs: function (data) {
    ...
    for (i = 0, l = oldIDs.length; i < l; i++) {
      uniqueID = oldIDs[i];
      if (
        Array.indexOf(refreshedIDs, uniqueID) === -1 &&
    ...
    return new Sys.WebForms.PageLoadingEventArgs(
      updated,
      deleted,
      data.dataItems,
    );
    ...
    while (element) {
      if (element.id) {
        ...
        if (
          ...
          proposedSettings = this._createPostBackSettings(
            true,
            null,
            elementUniqueID,
            originalElement,
          ...
          if (
            ...
            var indexOfPanel = Array.indexOf(
  ...
  _getScrollPosition: function () {
    ...
    if (
      d &&
      (this._validPosition(d.scrollLeft) || this._validPosition(d.scrollTop))
    ) {
      return {
        x: d.scrollLeft,
        y: d.scrollTop,
  ...
  _onFormSubmit: function (evt) {
    ...
    request.add_completed(
      Function.createDelegate(this, this._onFormSubmitCompleted),
    );
    ...
    if (handler) {
      ...
      eventArgs = new Sys.WebForms.InitializeRequestEventArgs(
        request,
    ...
    if (handler) {
      eventArgs = new Sys.WebForms.BeginRequestEventArgs(
        request,
        this._postBackSettings.sourceElement,
        panelsToUpdate || this._postBackSettings.panelsToUpdate,
      );
  ...
  _onFormSubmitCompleted: function (sender, eventArgs) {
    ...
    if (sender.get_timedOut()) {
      this._endPostBack(
        this._createPageRequestManagerTimeoutError(),
        sender,
        null,
      );
      return;
    ...
    if (sender.get_statusCode() !== 200) {
      this._endPostBack(
        this._createPageRequestManagerServerError(sender.get_statusCode()),
        sender,
        null,
      );
      return;
    ...
    if (
      data.asyncPostBackControlIDsNode &&
      data.postBackControlIDsNode &&
      data.updatePanelIDsNode &&
      data.panelsToRefreshNode &&
      data.childUpdatePanelIDsNode
    ) {
      var oldUpdatePanelIDs = this._updatePanelIDs,
        oldUpdatePanelClientIDs = this._updatePanelClientIDs;
      var childUpdatePanelIDsString = data.childUpdatePanelIDsNode.content;
      var childUpdatePanelIDs = childUpdatePanelIDsString.length
        ? childUpdatePanelIDsString.split(",")
        : [];

      var asyncPostBackControlIDsArray = this._splitNodeIntoArray(
        data.asyncPostBackControlIDsNode,
      );
      var postBackControlIDsArray = this._splitNodeIntoArray(
        data.postBackControlIDsNode,
      );
      var updatePanelIDsArray = this._splitNodeIntoArray(
        data.updatePanelIDsNode,
      );
      var panelsToRefreshIDs = this._splitNodeIntoArray(
        data.panelsToRefreshNode,
      );
        ...
        case "error":
          ...
          this._endPostBack(
            this._createPageRequestManagerServerError(
              Number.parseInvariant(deltaNode.id),
              deltaNode.content,
            ),
            executor,
            null,
  ...
  _scriptIncludesLoadComplete: function (scriptLoader, data) {
    ...
    this._commitControls(
      ...
      data.asyncPostBackTimeoutNode
        ? data.asyncPostBackTimeoutNode.content
        : null,
    );
    ...
    scriptLoader.loadScripts(
      ...
      Function.createDelegate(
        this,
        Function.createCallback(this._scriptsLoadComplete, data),
      ),
      null,
      null,
    );
  ...
  _updateControls: function (
    ...
    this._commitControls(
      this._processUpdatePanelArrays(
        updatePanelIDs,
        asyncPostBackControlIDs,
        postBackControlIDs,
        version4,
  ...
  _validPosition: function (position) {
    return (
      typeof position !== "undefined" && position !== null && position !== 0
    );
  },
};
...
Sys.WebForms.PageRequestManager.getInstance = function () {
  ...
  if (!prm) {
    prm = Sys.WebForms.PageRequestManager._instance =
