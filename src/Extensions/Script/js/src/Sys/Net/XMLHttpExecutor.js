...
Sys.Net.XMLHttpExecutor = function () {
  ...
  this._onReadyStateChange = function () {
    ...
    if (_this._xmlHttpRequest.readyState === 4 /*complete*/) {
      ...
      try {
        if (
          typeof _this._xmlHttpRequest.status === "undefined" ||
          _this._xmlHttpRequest.status === 0
        ) {
...
Sys.Net.XMLHttpExecutor.prototype = {
  ...
  executeRequest: function () {
    ...
    this._xmlHttpRequest.open(
      verb,
      this._webRequest.getResolvedUrl(),
      true /*async*/,
    );
  ...
  getAllResponseHeaders: function () {
    ...
    if (!this._responseAvailable) {
      throw Error.invalidOperation(
        String.format(
          Sys.Res.cannotCallBeforeResponse,
    ...
    if (!this._xmlHttpRequest) {
      throw Error.invalidOperation(
        String.format(
          Sys.Res.cannotCallOutsideHandler,
          "getAllResponseHeaders",
        ),
      );
    }
  ...
  get_xml: function () {
    ...
    // REVIEW: todo this used to use Sys.Runtime get_hostType
      ...
      typeof xml.setProperty != "undefined"
    ) {
