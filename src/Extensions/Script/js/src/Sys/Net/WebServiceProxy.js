...
Sys.Net.WebServiceProxy.invoke = function (
  ...
  var schemeHost =
      enableJsonp !== false
        ? Sys.Net.WebServiceProxy._xdomain.exec(servicePath)
        : null,
    tempCallback,
    jsonp =
      schemeHost &&
    ...
    function jsonpComplete(data, statusCode) {
      ...
      if (typeof statusCode !== "undefined" && statusCode !== 200) {
        if (onFailure) {
          ...
          error = String.format(
            error ? Sys.Res.webServiceFailed : Sys.Res.webServiceFailedNoMsg,
            methodName,
            error,
          );
  ...
  function onComplete(response, eventArgs) {
    if (response.get_responseAvailable()) {
      ...
      if (onFailure) {
        onFailure(
          new Sys.Net.WebServiceError(response.get_timedOut(), msg, "", ""),
          userContext,
          methodName,
        );
      }
