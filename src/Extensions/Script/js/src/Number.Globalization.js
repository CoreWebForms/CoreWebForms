...
Number._parse = function (value, cultureInfo) {
  ...
  var signInfo = Number._parseNumberNegativePattern(
    value,
    numFormat,
  ...
  if (decimalPos < 0) {
    ...
    fraction = intAndFraction.substr(
      decimalPos + numFormat.NumberDecimalSeparator.length,
    );
  }
  ...
  var altNumGroupSeparator = numFormat.NumberGroupSeparator.replace(
    /\u00A0/g,
    " ",
  );
  ...
  if (exponent !== null) {
    var expSignInfo = Number._parseNumberNegativePattern(
      exponent,
      numFormat,
      1,
    );
...
Number._parseNumberNegativePattern = function (
  value,
  numFormat,
