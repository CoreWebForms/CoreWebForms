  ...
  for (var i = 0; i < namespaceParts.length; i++) {
    ...
    if (nsType !== "undefined" && ns !== null) {
      if (nsType === "function") {
        throw Error.invalidOperation(
          String.format(
            Sys.Res.namespaceContainsClass,
      ...
      if (typeof ns !== "object" || ns instanceof Array) {
        throw Error.invalidOperation(
          String.format(
            Sys.Res.namespaceContainsNonObject,
            namespaceParts.splice(0, i + 1).join("."),
          ),
        );
      }
...
Type._checkDependency = function (dependency, featureName) {
  ...
  if (typeof featureName !== "undefined" && !isDependent) {
    throw Error.invalidOperation(
      String.format(
        Sys.Res.requiredScriptReferenceNotIncluded,
        featureName,
        dependency,
      ),
    );
...
Type._registerScript = function (scriptName, dependencies) {
  ...
  if (scripts[scriptName]) {
    throw Error.invalidOperation(
      String.format(Sys.Res.scriptAlreadyLoaded, scriptName),
    );
  }
  ...
  if (dependencies) {
    for (var i = 0, l = dependencies.length; i < l; i++) {
      ...
      if (!Type._checkDependency(dependency)) {
        throw Error.invalidOperation(
          String.format(
            Sys.Res.scriptDependencyNotFound,
            scriptName,
            dependency,
          ),
        );
