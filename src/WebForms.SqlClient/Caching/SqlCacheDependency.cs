...
internal static class SqlCookieExtensions
{
    private static object _index = new();

    public static string GetSqlDependencyCookie(this HttpContext context) => 
}
public sealed class SqlCacheDependency : CacheDependency
{
    ...
    public SqlCacheDependency(string databaseEntryName, string tableName)
        : base(null, [GetDependKey(databaseEntryName, tableName)])
    {
        Trace.WriteLine("SqlCacheDependency",
                        "Depend on key=" + GetDependKey(databaseEntryName, tableName) + "; value=" +
                        HttpRuntime.Cache.Get(GetDependKey(databaseEntryName, tableName)));
        ...
        object o = HttpRuntime.Cache.Get(GetDependKey(databaseEntryName, tableName));
        if (o == null)
        {
...
internal static class SqlCacheDependencyManager
{
    ...
    {
        return HttpRuntimeHelper.Services.GetRequiredService<IOptionsMonitor<SqlCacheDependencyDatabase>>().Get(database);
    }
    ...
    {
        var sqlDepDB = GetDatabaseConfig(database);
        string connectionString;

        // Return if polling isn't even enabled.            
        if (!sqlDepDB.Enabled)
        {
            throw new ConfigurationErrorsException(SR.GetString(SR.Polling_not_enabled_for_sql_cache));
        }

        // Return if the polltime is zero.  It means polling is disabled for this database.
        if (sqlDepDB.PollTime == 0)
        {
            throw new ConfigurationErrorsException(SR.GetString(SR.Polltime_zero_for_database_sql_cache, database));
        }
        ...
        {
            throw new ConfigurationErrorsException(SR.GetString(SR.Connection_string_not_found, sqlDepDB.ConnectionStringName));
        }
    ...
    {
#if PORT_APPLICATIONIMPERSONATION
            ...
            try
            {
                ...
                while (sqlReader.Read())
                {
                    ...
                    if (obj == null)
                    {
                        ...
                        cacheInternal.Add(monitorKey, changeId, dependencies: null, default, default, CacheItemPriority.NotRemovable, default);
                    ...
                    else if (changeId != (int)obj)
                    {
                        ...
                        cacheInternal.Insert(monitorKey, changeId, dependencies: null, default, default, CacheItemPriority.NotRemovable, default);
                    }
    ...
    static internal void EnsureTableIsRegisteredAndPolled(string database, string table)
    {
        ...
        if (HttpRuntime.Cache.Get(GetMoniterKey(database, table)) != null)
        {
            return;
        }
