...
internal class WebDirectoryBatchCompiler {
    ...
    private void AddBuildProviders(bool retryIfDeletionHappens) {
        ...
        foreach (VirtualFile vfile in _vdir.Files) {
            ...
            try {
                result = BuildManager.GetVPathBuildResultFromCache(vfile.VirtualPath);
            }
            catch {
            ...
            BuildProvider buildProvider = BuildManager.CreateBuildProvider(vfile.VirtualPath,
        ...
        if (DiskBuildResultCache.InUseAssemblyWasDeleted) {
            ...
            //     Debug.Write("WebDirectoryBatchCompiler", "Rerunning AddBuildProviders for '" +
...
//                     Debug.Write("BuildManager", buildProvider.VirtualPath + " Delay Load Assembly");
//                 } else {
//                     Debug.Write("BuildManager", buildProvider.VirtualPath + results.CompiledAssembly.EscapedCodeBase);
//                 }
//             }
//             else {
//                 Debug.Write("BuildManager", buildProvider.VirtualPath + ": no assembly");
//             }
// #endif
//         }
    }
    ...
    private void ProcessDependencies() {
        ...
        int i = 0;
        foreach (ICollection buildProviders in _nonDependentBuckets) {
            Debug.Write("BuildManager", String.Empty);
            Debug.Write("BuildManager", "Bucket " + i + " contains " + buildProviders.Count + " files");

            foreach (BuildProvider buildProvider in buildProviders)
                Debug.Write("BuildManager", buildProvider.VirtualPath);
            i++;
        }
#endif
            ...
            foreach (BuildProvider languageFreeBuildProvider in languageFreeBuildProviders) {
                ...
                if (defaultAssemblyBuilder == null || defaultAssemblyBuilder.IsBatchFull ||
                    ...
                    defaultAssemblyBuilder = CompilerType.GetDefaultAssemblyBuilder(
                        _compConfig, _referencedAssemblies, _vdir.VirtualPath /*configPath*/,
