...
// Flags that drive the behavior of precompilation
    ...
    public sealed class BuildManager {
        ...
        internal const string GlobalAsaxAssemblyName = AssemblyNamePrefix + "GlobalAsax";
        ...
        internal static bool InitializeBuildManager() {
            ...
            if (!_theBuildManagerInitialized) {
                ...
                Debug.WriteLine("BuildManager", "InitializeBuildManager ");
                // TODO: Migration
        ...
        private void Initialize() {
            ...
            _globalAsaxVirtualPath = new VirtualPath(HttpRuntime.AppDomainAppVirtualPath).Combine("Global.asax");

            _webHashFilePath = Path.Combine(HttpRuntime2.CodegenDirInternal, "hash\\hash.web");
            ...
            _excludedTopLevelDirectories.Add(HttpRuntime2.BinDirectoryName);
            _excludedTopLevelDirectories.Add(HttpRuntime2.CodeDirectoryName);
            _excludedTopLevelDirectories.Add(HttpRuntime2.ResourcesDirectoryName);
            _excludedTopLevelDirectories.Add(HttpRuntime2.LocalResourcesDirectoryName);
            _excludedTopLevelDirectories.Add(HttpRuntime2.WebRefDirectoryName);
            _excludedTopLevelDirectories.Add(HttpRuntime2.ThemesDirectoryName);
            ...
            _forbiddenTopLevelDirectories.Add(HttpRuntime2.CodeDirectoryName);
            _forbiddenTopLevelDirectories.Add(HttpRuntime2.ResourcesDirectoryName);
            _forbiddenTopLevelDirectories.Add(HttpRuntime2.LocalResourcesDirectoryName);
            _forbiddenTopLevelDirectories.Add(HttpRuntime2.WebRefDirectoryName);
            _forbiddenTopLevelDirectories.Add(HttpRuntime2.ThemesDirectoryName);
        ...
        private void RegularAppRuntimeModeInitialize() {
            ...
            _codeGenCache = new StandardDiskBuildResultCache(HttpRuntime2.CodegenDirInternal);
        ...
        private void PrecompiledAppRuntimeModeInitialize() {
            ...
            BuildResultCache preCompCache = new PrecompiledSiteDiskBuildResultCache(
                HttpRuntime2.BinDirectoryInternal);
        ...
        private void PrecompilationModeInitialize() {
            ...
            string targetBinDir = Path.Combine(_precompTargetPhysicalDir, HttpRuntime2.BinDirectoryName);
        ...
        private void LoadLicensesAssemblyIfExists() {
            string licAssemblyPath = Path.Combine(HttpRuntime2.BinDirectoryInternal, LicensesAssemblyName + ".dll");
        ...
        private long CheckTopLevelFilesUpToDateInternal(long cachedHash) {
            Debug.WriteLine("BuildManager", "specialFilesCombinedHash=" + cachedHash);
            ...
            specialFilesHashCodeCombiner.AddObject(HttpRuntime2.AppDomainAppPathInternal);
            ...
            if (!OptimizeCompilations) {
                ...
                string binPhysicalDir = HttpRuntime2.BinDirectoryInternal;
                ...
                specialFilesHashCodeCombiner.AddResourcesDirectory(HttpRuntime2.ResourcesDirectoryVirtualPath.MapPathInternal());

                specialFilesHashCodeCombiner.AddDirectory(HttpRuntime2.WebRefDirectoryVirtualPath.MapPathInternal());

                specialFilesHashCodeCombiner.AddDirectory(HttpRuntime2.CodeDirectoryVirtualPath.MapPathInternal());
        ...
        private void AfterPreAppStartExecute(Tuple<long, long> currentHash, Tuple<long, long> cachedTopLevelFilesHash) {
            ...
            try {
                ...
                {
                    Path = Path.GetDirectoryName(_webHashFilePath),
                    Filter = Path.GetFileName(_webHashFilePath)
                };

                fileSystemWatcher.Changed += OnWebHashFileChange;
        ...
        private void CheckCodeGenFiles(long currentHash, long cachedTopLevelFilesHash) {
            ...
            if (PrecompilingForCleanBuild || currentHash != cachedTopLevelFilesHash) {
                if (PrecompilingForCleanBuild) {
                    Debug.WriteLine("BuildManager", "Precompiling for clean build.");
                }
                else {
                    Debug.WriteLine("BuildManager", "EnsureFirstTimeInit: hash codes don't match.  Old=" +
                ...
                Debug.WriteLine("BuildManager", "BuildManager: the special files are up to date");
            }
        }

        private void OnWebHashFileChange(Object sender, FileSystemEventArgs e) {
            // Shutdown the app domain
            // TODO: Migration
            // Debug.WriteLine("BuildManager", _webHashFilePath + " changed - shutting down the app domain");
            // Debug.WriteLine("AppDomainFactory", "Shutting down appdomain because " + _webHashFilePath + " file changed");
        ...
        internal static void ExecutePreAppStart() {
            ...
            string preStartInitListPath = Path.Combine(HttpRuntime2.CodegenDirInternal, "preStartInitList.web");
        ...
        private Assembly CompileCodeDirectory(VirtualPath virtualDir, CodeDirectoryType dirType,
            ...
            Debug.WriteLine("BuildManager", "CompileCodeDirectory(" + virtualDir.VirtualPathString + ")");
            ...
            Debug.WriteLine("BuildManager", "CompileCodeDirectory generated assembly: " +
        ...
        private void CompileWebRefDirectory() {
            ...
            CompileCodeDirectory(HttpRuntime2.WebRefDirectoryVirtualPath, CodeDirectoryType.WebReferences,
        ...
        private void CompileCodeDirectories() {
            ...
            VirtualPath virtualDir = HttpRuntime2.CodeDirectoryVirtualPath;
        ...
        */
        ...
        internal static BuildProvider CreateBuildProvider(VirtualPath virtualPath,
            ...
            object o = HttpRuntime2.CreatePublicInstanceByWebObjectActivator(buildProviderType);
        ...
        private void ValidateVirtualPathInternal(VirtualPath virtualPath, bool allowCrossApp, bool codeFile) {
            ...
            if (HttpRuntime.AppDomainAppVirtualPath == virtualPath)
                return;

            int appPathLen = HttpRuntime.AppDomainAppVirtualPath.Length;
        ...
        private BuildResult GetVPathBuildResultInternal(VirtualPath virtualPath, bool noBuild, bool allowCrossApp, bool allowBuildInPrecompile, bool throwIfNotFound, bool ensureIsUpToDate = true) {
            ...
            Debug.WriteLine("BuildManager", "GetBuildResult(" + virtualPath + ")");
        ...
        private void EnsureFirstTimeDirectoryInit(VirtualPath virtualDir) {
            ...
            Debug.WriteLine("BuildManager", "EnsureFirstTimeDirectoryInit(" + virtualDir + ")");

            // Get the virtual path to the LocalResources subdirectory for this directory
            VirtualPath localResDir = virtualDir.SimpleCombineWithDir(HttpRuntime2.LocalResourcesDirectoryName);
            ...
            Debug.WriteLine("BuildManager", "EnsureFirstTimeDirectoryInit: dirExists=" + dirExists);

            try {
                // Monitor changes to it so the appdomain can shut down when it changes
                // TODO: Migration
                string localResDirPath = localResDir.MapPath();

                if (Directory.Exists(localResDirPath))
                {
                    FileSystemWatcher watcher = new FileSystemWatcher();
                    watcher.Path = localResDirPath;

                    // Watch for changes in LastAccess and LastWrite times, and the renaming of files or directories.
                    watcher.NotifyFilter = NotifyFilters.LastAccess | NotifyFilters.LastWrite | NotifyFilters.FileName | NotifyFilters.DirectoryName;

                    // Add event handlers.
                    watcher.Changed += OnLocalResourcesChanged;
                    watcher.Created += OnLocalResourcesChanged;
                    watcher.Deleted += OnLocalResourcesChanged;
                    watcher.Renamed += OnLocalResourcesChanged;
        ...
        private bool BatchCompileWebDirectory(VirtualDirectory vdir, VirtualPath virtualDir, bool ignoreErrors) {
            ...
            if (virtualDir == null)
                virtualDir = vdir.VirtualPath;

            if (vdir == null)
                vdir = virtualDir.GetDirectory();
        ...
        private void GetCodeDirectoryInformationInternal(VirtualPath virtualCodeDir,
            ...
            if (virtualCodeDir == HttpRuntime2.CodeDirectoryVirtualPath) {
            ...
            else if (virtualCodeDir == HttpRuntime2.ResourcesDirectoryVirtualPath) {
            ...
            // If virtualCodeDir is a subdir of WebReference virtual path.
                ...
                HttpRuntime2.WebRefDirectoryVirtualPath.VirtualPathString, 0, HttpRuntime2.WebRefDirectoryVirtualPath.VirtualPathString.Length,
                StringComparison.OrdinalIgnoreCase) == 0) {

                // Use the top WebReference directory info for its sub directories.
                virtualCodeDir = HttpRuntime2.WebRefDirectoryVirtualPath;
                dirType = CodeDirectoryType.WebReferences;

                _compilationStage = CompilationStage.TopLevelFiles;
            }
            else if (String.Compare(virtualCodeDir.FileName, HttpRuntime2.LocalResourcesDirectoryName,
                StringComparison.OrdinalIgnoreCase) == 0) {
    ...
    private static void GetGhostedVirtualPath(ref string virtualPath) {
        ...
        private BuildResult GetBuildResultFromCacheInternal(string cacheKey, bool keyFromVPP,
            ...
            //Debug.WriteLine("BuildManager", "GetBuildResultFromCacheInternal " + _theBuildManagerInitialized);
            if (!_theBuildManagerInitialized)
                return null;
            ...
            Debug.WriteLine("BuildManager", "Didn't find '" + virtualPath + "' in memory cache before lock");
            ...
            lock (this) {
                ...
                Debug.WriteLine("BuildManager", "Found '" + virtualPath + "' in " + _caches[i]);
        ...
        private bool DeletePrecompTargetDirectory() {
            try {
                ...
                Debug.WriteLine("BuildManager", "DeletePrecompTargetDirectory failed: " + e.Message);
            }
#else
            catch { }
#endif
        ...
        internal void PrecompileApp(ClientBuildManagerCallback callback, IEnumerable<string> excludedVirtualPaths) {
            ...
            try {
                ...
                PrecompileApp(HttpRuntime.AppDomainAppVirtualPath, excludedVirtualPaths);
            }
            finally {
        ...
        private void PrecompileAppInternal(VirtualPath startingVirtualDir, IEnumerable<string> excludedVirtualPaths) {
            ...
            if (_precompTargetPhysicalDir != null) {
                string targetBinDir = Path.Combine(_precompTargetPhysicalDir, HttpRuntime2.BinDirectoryName);
                CopyCompiledAssembliesToDestinationBin(HttpRuntime2.CodegenDirInternal, targetBinDir);
            }
        ...
        private bool IsPrecompiledAppInternal {
            get {
                if (!_isPrecompiledAppComputed) {
                    _isPrecompiledApp = ReadPrecompMarkerFile(HttpRuntime2.AppDomainAppPathInternal,
        ...
        private void PrecompileThemeDirectories() {
            string appPhysicalDir = Path.Combine(HttpRuntime2.AppDomainAppPathInternal, HttpRuntime2.ThemesDirectoryName);
        ...
        private void CopyStaticFilesRecursive(VirtualDirectory sourceVdir, string destPhysicalDir,
            ...
            foreach (VirtualFileBase child in sourceVdir.Children) {
                ...
                if (child.IsDirectory) {
                    ...
                    if (topLevel &&
                        (StringUtil.EqualsIgnoreCase(child.Name, HttpRuntime2.CodeDirectoryName) ||
                        StringUtil.EqualsIgnoreCase(child.Name, HttpRuntime2.ResourcesDirectoryName) ||
                        StringUtil.EqualsIgnoreCase(child.Name, HttpRuntime2.WebRefDirectoryName))) {
                    ...
                    if (!PrecompilingForUpdatableDeployment && StringUtil.EqualsIgnoreCase(child.Name,
                            HttpRuntime2.LocalResourcesDirectoryName)) {
                        continue;
                    }
        ...
        private static string UserCachePath {
            get {
                if (_userCachePath == null) {
                    ...
                    string userCachePath = Path.Combine(HttpRuntime2.CodegenDirInternal, "UserCache");
