...
public class ControlBuilder
{
    ...
    private static readonly Regex databindRegex = new(@"\G\s*<%\s*?#(?<encode>:)?(?<code>.*?)?%>\s*\z", RegexOptions.Multiline | RegexOptions.Singleline);
    internal static readonly Regex expressionBuilderRegex = new("\\G\\s*<%\\s*\\$\\s*(?<code>.*)?%>\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
    private static readonly Regex bindExpressionRegex = new("^\\s*bind\\s*\\((?<params>.*)\\)\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
    private static readonly Regex bindParametersRegex = new("\\s*((\"(?<fieldName>(([\\w\\.]+)|(\\[.+\\])))\")|('(?<fieldName>(([\\w\\.]+)|(\\[.+\\])))'))\\s*(,\\s*((\"(?<formatString>.*)\")|('(?<formatString>.*)'))\\s*)?\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
    private static readonly Regex bindItemExpressionRegex = new("^\\s*BindItem\\.(?<params>.*)\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
    private static readonly Regex bindItemParametersRegex = new("(?<fieldName>([\\w\\.]+))\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
    private static readonly Regex evalExpressionRegex = new("^\\s*eval\\s*\\((?<params>.*)\\)\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
    private static readonly Regex formatStringRegex = new("^(([^\"]*(\"\")?)*)$", RegexOptions.Multiline | RegexOptions.Singleline);
...
#pragma warning restore 0649
    ...
    /// </devdoc>
    internal static TemplateControl TemplateControl
    {
        get
        {
    ...
    private static bool HasFilteredEntries(ICollection entries)
    {
        foreach (PropertyEntry entry in entries)
        {
    ...
    private static readonly ParseChildrenAttribute s_markerParseChildrenAttribute = new ParseChildrenAttribute();

    private static readonly Hashtable s_parseChildrenAttributeCache = new Hashtable();
    ...
    private static readonly IWebObjectFactory s_defaultControlBuilderFactory = new DefaultControlBuilderFactory();
...
#pragma warning restore 0649
    ...
    {
        readonly IFilterResolutionService _filterResolutionService;
