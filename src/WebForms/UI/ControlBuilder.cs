...
namespace System.Web.UI
{
    ...
    public class ControlBuilder
    {
        ...
        private readonly static Regex databindRegex = new(@"\G\s*<%\s*?#(?<encode>:)?(?<code>.*?)?%>\s*\z", RegexOptions.Multiline | RegexOptions.Singleline);
        internal readonly static Regex expressionBuilderRegex = new("\\G\\s*<%\\s*\\$\\s*(?<code>.*)?%>\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
        private readonly static Regex bindExpressionRegex = new("^\\s*bind\\s*\\((?<params>.*)\\)\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
        private readonly static Regex bindParametersRegex = new("\\s*((\"(?<fieldName>(([\\w\\.]+)|(\\[.+\\])))\")|('(?<fieldName>(([\\w\\.]+)|(\\[.+\\])))'))\\s*(,\\s*((\"(?<formatString>.*)\")|('(?<formatString>.*)'))\\s*)?\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
        private readonly static Regex bindItemExpressionRegex = new("^\\s*BindItem\\.(?<params>.*)\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
        private readonly static Regex bindItemParametersRegex = new("(?<fieldName>([\\w\\.]+))\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
        private readonly static Regex evalExpressionRegex = new("^\\s*eval\\s*\\((?<params>.*)\\)\\s*\\z", RegexOptions.Multiline | RegexOptions.Singleline);
        private readonly static Regex formatStringRegex = new("^(([^\"]*(\"\")?)*)$", RegexOptions.Multiline | RegexOptions.Singleline);
