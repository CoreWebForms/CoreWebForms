...
public class Page : TemplateControl, IHttpAsyncHandler
{
    ...
    internal const int DefaultMaxPageStateFieldLength = -1;
    internal const int DefaultAsyncTimeoutSeconds = 45;
...
#pragma warning restore 0649
    ...
    internal static bool ClientSupportsFocus => true;

    internal static bool ClientSupportsJavaScript => true;
...
#else
                _xhtmlConformanceMode = System.Web.Configuration.XhtmlConformanceMode.Strict;
#endif
    ...
    internal uint GetClientStateIdentifier()
    {
        ...
        int pageHashCode = StringUtil.GetNonRandomizedHashCode(GetType().Name, ignoreCase: true);
...
#else
        throw new HttpUnhandledException("Error constructing page", e);
#endif
    }
...
#pragma warning restore 618 // calling obsolete methods
...
#else
        var adapter = (HttpContextCore)Context;

        if (string.Equals(_request.HttpMethod, "POST", StringComparison.OrdinalIgnoreCase))
        {
            return (dontReturnNull || adapter.Request.HasFormContentType) ? _request.Form : null;
        }
        else
        {
    ...
    internal const String RedirectQueryStringValue = "1";
    ...
    protected internal virtual NameValueCollection DeterminePostBackMode()
    {
        ...
        else if (Request.QueryString[RedirectQueryStringVariable] == RedirectQueryStringValue)
        {
            ret = null;
        }
...
#endif
        return GetCollectionBasedOnMethod(true);
    }
...
*/
    ...
    internal void LoadScrollPosition()
    {
        ...
        if (_requestValueCollection != null)
        {
            ...
            if (xpos != null)
            {
                _scrollPositionX = HttpUtility2.TryParseCoordinates(xpos, out doubleValue) ? (int)doubleValue : 0;
            }
            string ypos = _requestValueCollection[_scrollPositionYID];
            if (ypos != null)
            {
                _scrollPositionY = HttpUtility2.TryParseCoordinates(ypos, out doubleValue) ? (int)doubleValue : 0;
            }
        }
    }
...
" : String.Empty), true);
    ...
    internal bool ShouldLoadControlState(Control control)
    {
        ...
        {
            if (control != cleared && control.IsDescendentOf(cleared))
            {
                return false;
            }
        }
            ...
            foreach (string postKey in postData)
            {
                if (postKey != null)
                {
                    ...
                    if (consumer != null)
                    {
                        ...
                        bool changed = await LoadPostDataAsync(consumer, postKey, postCollection).ConfigureAwait(false);
        ...
        if (_controlsRequiringPostBack != null)
        {
            foreach (string controlID in _controlsRequiringPostBack)
            {
                ...
                if (c != null)
                {
                    ...
                    bool changed = await LoadPostDataAsync(consumer, controlID, postCollection).ConfigureAwait(false);
                    if (changed)
                    {
    ...
    static void SetCulture(Thread currentThread, CultureInfo currentCulture, CultureInfo currentUICulture)
    {
    ...
    private static void ProcessRequestEndTrace()
    {
    }
...
*/
    ...
    private async Task ProcessRequestAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            await preWorkTask.ConfigureAwait(false);
            ...
            try
            {
                using (CancellationTokenSource cancellationTokenSource = CreateCancellationTokenFromAsyncTimeout())
                {
                    ...
                    try
                    {
                        await _asyncTaskManager.ExecuteTasksAsync(this, EventArgs.Empty, cancellationToken).ConfigureAwait(false);
                    }
                    finally
                    {
            ...
            await postWorkTask.ConfigureAwait(false);
        }
        finally
        {
            // call Unload events
    ...
    public void AddOnPreRenderCompleteAsync(BeginEventHandler beginHandler, EndEventHandler endHandler, Object state)
    {
        ...
        {
            throw new ArgumentNullException(nameof(beginHandler));
        }

        if (endHandler == null)
        {
            throw new ArgumentNullException(nameof(endHandler));
        }
