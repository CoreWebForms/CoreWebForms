...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    public sealed class WebPartCollection : ReadOnlyCollectionBase {
        ...
        public WebPartCollection(ICollection webParts) {
            if (webParts == null) {
                throw new ArgumentNullException(nameof(webParts));
            }

            foreach (object obj in webParts) {
                if (obj == null) {
                    throw new ArgumentException(SR.GetString(SR.Collection_CantAddNull), nameof(webParts));
                }
                if (!(obj is WebPart)) {
                    throw new ArgumentException(SR.GetString(SR.Collection_InvalidType, "WebPart"), nameof(webParts));
                }
                InnerList.Add(obj);
            }
        }
