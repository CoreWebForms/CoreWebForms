        ...
        private void Initialize(TransformerTypeCollection existingTransformerTypes, ICollection transformerTypes) {
            ...
            if (transformerTypes != null) {
                foreach (object obj in transformerTypes) {
                    if (obj == null) {
                        throw new ArgumentException(SR.GetString(SR.Collection_CantAddNull), nameof(transformerTypes));
                    }
                    if (!(obj is Type)) {
                        throw new ArgumentException(SR.GetString(SR.Collection_InvalidType, "Type"), nameof(transformerTypes));
                    }
