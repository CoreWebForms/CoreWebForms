        ...
        private void Initialize(ConnectionInterfaceCollection existingConnectionInterfaces, ICollection connectionInterfaces) {
            ...
            if (connectionInterfaces != null) {
                foreach (object obj in connectionInterfaces) {
                    if (obj == null) {
                        throw new ArgumentException(SR.GetString(SR.Collection_CantAddNull), nameof(connectionInterfaces));
                    }
                    if (!(obj is Type)) {
                        throw new ArgumentException(SR.GetString(SR.Collection_InvalidType, "Type"), nameof(connectionInterfaces));
                    }
                    InnerList.Add(obj);
                }
            }
