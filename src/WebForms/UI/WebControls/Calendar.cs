...
namespace System.Web.UI.WebControls
{
    ...
    public class Calendar : WebControl, IPostBackEventHandler
    {
        ...
        /// <devdoc>
        /// <para>Initializes a new instance of the <see cref='Calendar'/> class.</para>
        /// </devdoc>
        ...
        public DayNameFormat DayNameFormat
        {
            get
            {
                var dnf = ViewState["DayNameFormat"];
        ...
        public FirstDayOfWeek FirstDayOfWeek
        {
            get
            {
                var o = ViewState["FirstDayOfWeek"];
        ...
        ///       navigation hyperlink if the <see cref='ShowNextPrevMonth'/> property is set to
        ...
        public NextPrevFormat NextPrevFormat
        {
            get
            {
                var npf = ViewState["NextPrevFormat"];
        ...
        /// <devdoc>
        /// <para>Gets a collection of <see cref='DateTime' qualify='true'/> objects representing days selected on the <see cref='Calendar'/>. This
        ...
        public CalendarSelectionMode SelectionMode
        {
            get
            {
                var csm = ViewState["SelectionMode"];
        ...
        ///       the selector column if <see cref='SelectionMode'/> is
        ...
        public string SelectMonthText
        {
            get
            {
                var s = ViewState["SelectMonthText"];
                return ((s == null) ? "&gt;&gt;" : (string)s);
            }
            set
            {
        ...
        public string SelectWeekText
        {
            get
            {
                var s = ViewState["SelectWeekText"];
                return ((s == null) ? "&gt;" : (string)s);
            }
            set
            {
        ...
        /// <devdoc>
        /// <para>Gets or sets a value indicating whether the <see cref='Calendar'/>
        ...
        public bool ShowNextPrevMonth
        {
            get
            {
                var b = ViewState["ShowNextPrevMonth"];
                return ((b == null) ? true : (bool)b);
            }
            set
            {
        ...
        /// <devdoc>
        /// <para>Gets the style properties of the <see cref='Calendar'/> title. This property is
        ///    read-only.</para>
        /// </devdoc>
        [
        WebCategory("Styles"),
        ...
        public virtual bool UseAccessibleHeader
        {
            get
            {
                var o = ViewState["UseAccessibleHeader"];
        ...
        /// <devdoc>
        /// <para>Occurs when each day is created in teh control hierarchy for the <see cref='Calendar'/>.</para>
        /// </devdoc>
        [
        WebCategory("Action"),
        ...
        ///       selector and changes the <see cref='SelectedDate'/>.</para>
        /// </devdoc>
        [
        WebCategory("Action"),
        ...
        ///       user clicks on the next or previous month <see cref='Button'/> controls on the title.</para>
        /// </devdoc>
        [
        WebCategory("Action"),
        ...
        {
            var firstDayOfMonth = visibleDate;
            ...
            var daysFromLastMonth = ((int)threadCalendar.GetDayOfWeek(firstDayOfMonth)) - NumericFirstDayOfWeek();
        ...
        private int GetDefinedStyleMask()
        {
            ...
            var styleMask = STYLEMASK_SELECTED;
        ...
        /// <devdoc>
        /// </devdoc>
        private static string GetMonthName(int m, bool bFull)
        {
            if (bFull)
            {
        ...
        /// <devdoc>
        /// <para>Determines if a <see cref='CalendarSelectionMode'/>
        ...
        /// <internalonly/>
        /// <devdoc>
        /// <para>Loads a saved state of the <see cref='Calendar'/>. </para>
        /// </devdoc>
        protected override void LoadViewState(object savedState)
        {
            if (savedState != null)
            {
                ...
                var selDates = (ArrayList)ViewState["SD"];
                if (selDates != null)
                {
                    dateList = selDates;
        ...
        /// <devdoc>
        /// <para>Raises the <see langword='DayRender '/>event for a <see cref='Calendar'/>.</para>
        /// </devdoc>
        protected virtual void OnDayRender(TableCell cell, CalendarDay day)
        {
            var handler = (DayRenderEventHandler)Events[EventDayRender];
            if (handler != null)
            {
                var absoluteDay = day.Date.Subtract(baseDate).Days;
                ...
                if (page != null)
                {
                    var eventArgument = absoluteDay.ToString(CultureInfo.InvariantCulture);
        ...
        /// <devdoc>
        /// <para>Raises the <see langword='SelectionChanged '/>event for a <see cref='Calendar'/>.</para>
        /// </devdoc>
        protected virtual void OnSelectionChanged()
        {
            var handler = (EventHandler)Events[EventSelectionChanged];
            if (handler != null)
            {
        ...
        /// <devdoc>
        /// <para>Raises the <see langword='VisibleMonthChanged '/>event for a <see cref='Calendar'/>.</para>
        /// </devdoc>
        protected virtual void OnVisibleMonthChanged(DateTime newDate, DateTime previousDate)
        {
            var handler = (MonthChangedEventHandler)Events[EventVisibleMonthChanged];
            if (handler != null)
            {
        ...
        /// <internalonly/>
        /// <devdoc>
        /// <para>Raises events on post back for the <see cref='Calendar'/> control.</para>
        /// </devdoc>
        protected virtual void RaisePostBackEvent(string eventArgument)
        {
            ...
            {
                var pbeh = AdapterInternal as IPostBackEventHandler;
                if (pbeh != null)
                {
            ...
            else
            {
                ...
                if (string.Compare(eventArgument, 0, NAVIGATE_MONTH_COMMAND, 0, NAVIGATE_MONTH_COMMAND.Length, StringComparison.Ordinal) == 0)
                {
                    ...
                    // base date.
                    var oldDate = VisibleDate;
                    if (oldDate.Equals(DateTime.MinValue))
                    {
                        oldDate = TodaysDate;
                    }
                    var newDateDiff = int.Parse(eventArgument.AsSpan(NAVIGATE_MONTH_COMMAND.Length), CultureInfo.InvariantCulture);
                ...
                else if (string.Compare(eventArgument, 0, SELECT_RANGE_COMMAND, 0, SELECT_RANGE_COMMAND.Length, StringComparison.Ordinal) == 0)
                {
                    ...
                    var rangeValue = int.Parse(eventArgument.AsSpan(SELECT_RANGE_COMMAND.Length), CultureInfo.InvariantCulture);
                    var dayDiff = rangeValue / 100;
                    var dayRange = rangeValue % 100;
                    if (dayRange < 1)
                    {
                        dayRange = 100 + dayRange;
                        dayDiff -= 1;
                    }
                    var dt = baseDate.AddDays(dayDiff);
                ...
                else
                {
                    ...
                    var dayDiff = int.Parse(eventArgument, CultureInfo.InvariantCulture);
                    var dt = baseDate.AddDays(dayDiff);
                    SelectRange(dt, dt);
                }
            }
        ...
        /// <internalonly/>
        /// <devdoc>
        /// <para>Displays the <see cref='Calendar'/> control on the client.</para>
        /// </devdoc>
        protected internal override void Render(HtmlTextWriter writer)
        {
            ...
            var firstDay = FirstCalendarDay(visibleDate);
            ...
            var useAccessibleHeader = UseAccessibleHeader;
        ...
        private void RenderDayHeader(HtmlTextWriter writer, DateTime visibleDate, CalendarSelectionMode selectionMode, bool buttonsActive, bool useAccessibleHeader)
        {
            ...
            {
                var monthSelectorStyle = new TableItemStyle();
                ...
                if (selectionMode == CalendarSelectionMode.DayWeekMonth)
                {
                    ...
                    // the result is the day difference from the base date of the first day, and the remainder is the
                    // number of days to select.
                    var startOffset = visibleDate.Subtract(baseDate).Days;
                    var monthLength = threadCalendar.GetDaysInMonth(threadCalendar.GetYear(visibleDate), threadCalendar.GetMonth(visibleDate), threadCalendar.GetEra(visibleDate));
                    ...
                    var monthSelectKey = SELECT_RANGE_COMMAND + ((startOffset * 100) + monthLength).ToString(CultureInfo.InvariantCulture);
            ...
            var dayNameStyle = new TableItemStyle();
            dayNameStyle.HorizontalAlign = HorizontalAlign.Center;
            dayNameStyle.CopyFrom(DayHeaderStyle);
            var dayNameFormat = DayNameFormat;

            var numericFirstDay = NumericFirstDayOfWeek();
            for (var i = numericFirstDay; i < numericFirstDay + 7; i++)
            {
                string dayName;
                var dayOfWeek = i % 7;
                ...
                if (useAccessibleHeader)
                {
                    var fullDayName = dtf.GetDayName((DayOfWeek)dayOfWeek);
        ...
        private void RenderDays(HtmlTextWriter writer, DateTime firstDay, DateTime visibleDate, CalendarSelectionMode selectionMode, bool buttonsActive, bool useAccessibleHeader)
        {
            ...
            Unit defaultWidth;
            var hasWeekSelectors = HasWeekSelectors(selectionMode);
            if (hasWeekSelectors)
            {
            ...
            var usesStandardDayDigits = !(threadCalendar is HebrewCalendar);

            // This determines whether we can write out cells directly, or whether we have to create whole
            // TableCell objects for each day.
            var hasRenderEvent = (this.GetType() != typeof(Calendar)
                                   || Events[EventDayRender] != null);

            var cellStyles = new TableItemStyle[16];
            var definedStyleMask = GetDefinedStyleMask();
            var todaysDate = TodaysDate;
            var selectWeekText = SelectWeekText;
            var daysSelectable = buttonsActive && (selectionMode != CalendarSelectionMode.None);
            var visibleDateMonth = threadCalendar.GetMonth(visibleDate);
            var absoluteDay = firstDay.Subtract(baseDate).Days;
            ...
            var inDesignSelectionMode = (DesignMode && SelectionMode != CalendarSelectionMode.None);
            ...
            var passedLastSupportedDate = false;
            var secondLastMonth = threadCalendar.AddMonths(maxSupportedDate, -1);
            var lastOrSecondLastMonth = (IsMaxSupportedYearMonth(visibleDate) ||
            ...
            for (var iRow = 0; iRow < 6; iRow++)
            {
                ...
                if (hasWeekSelectors)
                {
                    ...
                    // the result is the day difference from the base date of the first day, and the remainder is the
                    // number of days to select.
                    var dayDiffParameter = (absoluteDay * 100) + 7;
                    ...
                    {
                        var daysFromLastDate = maxSupportedDate.Subtract(d).Days;
                        if (daysFromLastDate < 6)
                        {
                            dayDiffParameter -= (6 - daysFromLastDate);
                        }
                    }
                    var weekSelectKey = SELECT_RANGE_COMMAND + dayDiffParameter.ToString(CultureInfo.InvariantCulture);

                    string selectWeekTitle = null;
                    if (useAccessibleHeader)
                    {
                        var weekOfMonth = iRow + 1;
                ...
                for (var iDay = 0; iDay < 7; iDay++)
                {
                    ...
                    var dayOfWeek = (int)threadCalendar.GetDayOfWeek(d);
                    var dayOfMonth = threadCalendar.GetDayOfMonth(d);
                    ...
                    if (day.IsWeekend)
                        styleMask |= STYLEMASK_WEEKEND;
                    var dayStyleMask = definedStyleMask & styleMask;
                    // determine the unique portion of the mask for the current calendar,
                    // which will strip out the day style bit
                    var dayStyleID = dayStyleMask & STYLEMASK_UNIQUE;

                    var cellStyle = cellStyles[dayStyleID];
                    if (cellStyle == null)
                    {
                    ...
                    if (hasRenderEvent)
                    {
                        ...
                        var cdc = new TableCell();
                        cdc.ApplyStyle(cellStyle);

                        var dayContent = new LiteralControl(dayNumberText);
        ...
        private void RenderTitle(HtmlTextWriter writer, DateTime visibleDate, CalendarSelectionMode selectionMode, bool buttonsActive, bool useAccessibleHeader)
        {
            ...
            var nextPrevFormat = NextPrevFormat;
            ...
            if (ShowNextPrevMonth)
            {
                ...
                else
                {
                    ...
                    {
                        var monthNo = threadCalendar.GetMonth(threadCalendar.AddMonths(visibleDate, -1));
                    ...
                    var secondSupportedMonth = threadCalendar.AddMonths(minSupportedDate, 1);
                    ...
                    var prevMonthKey = NAVIGATE_MONTH_COMMAND + (prevMonthDate.Subtract(baseDate)).Days.ToString(CultureInfo.InvariantCulture);
            ...
            var cellMainStyle = new TableItemStyle();
            ...
            switch (TitleFormat)
            {
                ...
                case TitleFormat.MonthYear:
                    var titlePattern = DateTimeFormatInfo.CurrentInfo.YearMonthPattern;
            ...
            if (ShowNextPrevMonth)
            {
                ...
                else
                {
                    ...
                    // Month navigation. The command starts with a "V" and the remainder is day difference from the
                    // base date.
                    var nextMonthDate = threadCalendar.AddMonths(visibleDate, 1);
                    var nextMonthKey = NAVIGATE_MONTH_COMMAND + (nextMonthDate.Subtract(baseDate)).Days.ToString(CultureInfo.InvariantCulture);
