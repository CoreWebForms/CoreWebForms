...
public class ModelDataSourceView : DataSourceView, IStateManager
{
    ...
    private readonly ModelDataSource _owner;
    ...
    private static object GetPropertyValueByName(object o, string name)
    {
    ...
    private static void SetCancellationTokenIfRequired(ModelDataSourceMethod method, bool isAsyncMethod, CancellationToken? cancellationToken)
    {
    ...
    private void SelectAsync(DataSourceSelectArguments arguments, DataSourceViewSelectCallback callback, ModelDataSourceMethod method)
    {
        ...
        // function.
        SynchronizationContext.Current.PostAsync(func, null);
    }
    ...
    private Func<object, Task> GetSelectAsyncFunc(DataSourceSelectArguments arguments, DataSourceViewSelectCallback callback, ModelDataSourceMethod method)
    {
        Func<object, Task> func = async _ =>
        {
            ...
            if (result.ReturnValue != null)
            {
                await ((Task)result.ReturnValue).ConfigureAwait(false);
                ...
                else if (returnValue is SelectResult)
                {
                    ...
                    {
                        await viewOperationTask.ConfigureAwait(false);
                    }
    ...
    private static IValueProvider GetCustomValueProvider(ModelBindingExecutionContext modelBindingExecutionContext, ParameterInfo parameterInfo, ref string modelName, out bool validateRequest)
    {
        validateRequest = true;
        object[] valueProviderAttributes = parameterInfo.GetCustomAttributes(typeof(IValueProviderSource), false);

        if (valueProviderAttributes.Length > 1)
        {
        ...
        if (valueProviderAttributes.Length > 0)
        {
    ...
    private static void ValidateMethodIsCallable(MethodInfo methodInfo)
    {
    ...
    /// </summary>
    private static OrderedDictionary GetOutputParameters(ParameterInfo[] parameters, object[] values)
    {
    ...
    private static void ValidateParameterValue(ParameterInfo parameterInfo, object value, MethodInfo methodInfo)
    {
    ...
    private static Type ModelType
    {
        get
        {
#if PORT_MODELBINDING_BUILDMANAGER
