...
<script src=""";
    ...
    public void ValidateEvent(string uniqueId, string argument)
    {
        ...
        if (string.IsNullOrEmpty(uniqueId))
        {
            throw new ArgumentException(SR.GetString(SR.Parameter_NullOrEmpty, "uniqueId"), nameof(uniqueId));
        }
...
WebForm_InitCallback();", true);
    ...
    {
        // check paramters
        WebUtil.StringUtil.CheckAndTrimString(controlId, nameof(controlId));
        WebUtil.StringUtil.CheckAndTrimString(attributeName, nameof(attributeName));
    ...
    private static void RegisterScriptBlock(ScriptKey key, string script, ref ListDictionary scriptBlocks, ref ArrayList scriptList, bool needsScriptTags)
    {
        ...
        {
            Tuple<ScriptKey, string, bool> entry = new Tuple<ScriptKey, string, bool>(key, script, needsScriptTags);
...
internal class ScriptKey
{
    ...
    {
        return HashCode.Combine(_type, _key, _isInclude);
    }
