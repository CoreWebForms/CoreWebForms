...
namespace System.Web
{
    ...
    public class XmlSiteMapProvider : StaticSiteMapProvider, IDisposable
    {
        ...
        public XmlSiteMapProvider()
        {
            ...
            //_provider = HttpContext.Current.Request.AsAspNetCore().HttpContext.RequestServices.GetRequiredService<VirtualPathProvider>();
        }
        ...
        public override SiteMapNodeCollection GetChildNodes(SiteMapNode node)
        {
            ...
            if (context == null
                // ||!context.Response.UsePathModifier //TODO
                || subNodes.Count == 0)
            {
                return subNodes;
            }
        ...
        protected internal override void AddNode(SiteMapNode node, SiteMapNode parentNode)
        {
            ...
            {
                throw new ArgumentException(SR.GetString(SR.XmlSiteMapProvider_cannot_add_node, node.ToString()), nameof(node));
            }

            if (parentOwnerProvider != this)
            {
                throw new ArgumentException(SR.GetString(SR.XmlSiteMapProvider_cannot_add_node, parentNode.ToString()), nameof(parentNode));
            }
        ...
        public override SiteMapNode BuildSiteMap()
        {
            ...
            lock (_lock)
            {
                ...
                try
                {
                    using (Stream stream = _normalizedVirtualPath.OpenFile(_provider))
                    {
        ...
        private static void CheckSiteMapFileExists()
        {
            //TODO get webform provider, eventually it would throw exception
        ...
        private XmlDocument GetConfigDocument()
        {
            ...
            _filename = _normalizedVirtualPath.Path;
        ...
        private SiteMapNode GetNodeFromXmlNode(XmlNode xmlNode, Queue queue)
        {
            ...
            if (!String.IsNullOrEmpty(url))
            {
                ...
                if (!UrlPath.IsAbsolutePhysicalPath(url))
                {
                    ...
                    {
                        url = UrlPath.Combine(HttpRuntime.AppDomainAppVirtualPath, url);
                    }
                }
        ...
        private static SiteMapProvider GetProviderFromName(string providerName)
        {
            ...
            if (provider == null)
            {
                throw new ProviderException(SR.GetString("Provider_Not_Found", providerName));
            }
        ...
        private static SiteMapNode ApplyModifierIfExists(SiteMapNode node)
        {
            ...
            if (node == null || context == null
                // || !context.Response.UsePathModifier //TODO check
                )
            {
                return node;
            }
